  ...
  return (
    <Card className="overflow-hidden border-0 shadow-sm transition-all hover:shadow-md h-full">
      ...
      <CardContent className="p-4 sm:p-6">
        <Link
          href={`/products/${id || name.toLowerCase().replace(/\s+/g, "-")}`}
        >
          <h3 className="text-base sm:text-xl font-medium line-clamp-1">
            {name}
          </h3>
        </Link>
        <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-neutral-600 line-clamp-2">
          {description}
        </p>
      </CardContent>
      <CardFooter className="flex items-center justify-between flex-wrap gap-2 p-4 sm:p-6 pt-0">
